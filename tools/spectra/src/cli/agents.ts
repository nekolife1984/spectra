...
const buildGuideSteps = (agent: AgentType): string[] => {
  ...
  const steps: string[] = [
    ...
    'Tip: Update `{{SPECTRA_DIR}}/templates/` like `requirements.md`, `design.md`, and `tasks.md` so the generated steering and specs follow your team\'s and project\'s development process.',
    'Optional: run `.spectra/scripts/setup-crg.sh` to install code-review-graph for code-graph-aware spec→code impact analysis.',
  ];
