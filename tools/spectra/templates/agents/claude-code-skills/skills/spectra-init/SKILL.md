...
## Execution Steps
1. **Check for Brief**: If `{{SPECS_DIR}}/{feature-name}/brief.md` exists (created by `/spectra-discovery`), read it. The brief contains problem, approach, scope, and constraints from the discovery session. Use this to pre-fill the project description and skip clarification questions that the brief already answers.
...
6. **Initialize `.spectra/trace-mapping.yaml`** (if it doesn't exist):
   - If `.spectra/trace-mapping.yaml` doesn't exist in the project root, create it with a skeleton entry for this spec
   ...
   - Write a basic entry for each requirement:
     ...
     mappings:
       - id: "1.1"
         ...
         spec: "{{SPECS_DIR}}/{feature-name}/requirements.md"
         design: ""
         code:
           files: []
           symbols: []
         ...
         tags: ["@impl"]
     ```
   - If `.spectra/trace-mapping.yaml` already exists, append new entries without duplicating existing `id` values
