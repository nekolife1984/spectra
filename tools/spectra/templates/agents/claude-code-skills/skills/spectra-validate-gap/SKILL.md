...
5. **Write Gap Analysis to Disk**:
   ...
   **Write the gap analysis to disk so it survives session boundaries and can be referenced during design phase.**
   ...
   - Save the gap analysis to `{{SPECTRA_DIR}}/specs/$1/research.md`
...
### Error Scenarios
- **Missing Requirements**: If requirements.md doesn't exist, stop with message: "Run `$spectra-requirements $1` first to generate requirements"
...
- Run `$spectra-design $1` to create technical design document
- Or `$spectra-design $1 -y` to auto-approve requirements and proceed directly
