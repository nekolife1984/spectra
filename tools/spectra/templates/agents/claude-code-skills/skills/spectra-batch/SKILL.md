...
1. Read `{{SPECTRA_DIR}}/steering/roadmap.md`
...
4. For each pending feature in `## Specs (dependency order)`, verify `{{SPECS_DIR}}/<feature>/brief.md` exists
...
1. Read the brief at {{SPECS_DIR}}/{feature-name}/brief.md for feature context
...
3. Execute the full spec pipeline. For each phase, read the corresponding skill's SKILL.md for complete instructions (templates, rules, review gates):
   ...
   b. Generate requirements: Read .agents/skills/spectra-requirements/SKILL.md, then follow its steps
   c. Generate design: Read .agents/skills/spectra-design/SKILL.md, then follow its steps
   d. Generate tasks: Read .agents/skills/spectra-tasks/SKILL.md, then follow its steps
...
After all waves complete, spawn a **single sub-agent** for cross-spec consistency review. Use the `spectra-reviewer` custom agent if available (configured with `model = "gpt-5.4"` and `model_reasoning_effort = "high"` in `.codex/agents/spectra-reviewer.toml`). This is the highest-value quality gate -- it catches issues that per-spec review gates cannot.

**Sub-agent task**:

Read ALL generated specs and check for consistency across the entire project:
- `{{SPECS_DIR}}/*/design.md` (primary: contains interfaces, data models, architecture)
- `{{SPECS_DIR}}/*/requirements.md` (for scope and acceptance criteria)
- `{{SPECS_DIR}}/*/tasks.md` (for boundary annotations only -- read _Boundary:_ lines, skip task descriptions)
...
1. Scan `{{SPECS_DIR}}/*/tasks.md` to verify all specs exist
...
5. Auto-generate `.spectra/trace-mapping.yaml` skeleton entries:
   - For each completed spec, extract `requirements.md` requirement IDs
   - Create or append to `.spectra/trace-mapping.yaml` with basic entries (`id`, `spec`, `design`, empty `code.files`, empty `tasks`, `tags: ["@impl"]`)
   - If `.spectra/trace-mapping.yaml` exists, merge with existing entries (don't overwrite duplicate `id` values)
