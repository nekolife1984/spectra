...
- **Specs inventory**: Scan `{{SPECTRA_DIR}}/specs/*/spec.json` for `name`, `phase` fields and `approvals` status. Note feature names and their current status.
...
Write `{{SPECTRA_DIR}}/specs/<feature-name>/brief.md` to disk with this structure:
...
- `{{SPECTRA_DIR}}/specs/<feature>/brief.md` for every feature listed under `## Specs (dependency order)`
