...
## Execution Steps
1. **Check for Brief**: If `{{SPECTRA_DIR}}/specs/{feature-name}/brief.md` exists (created by `/spectra-discovery`), read it. The brief contains problem, approach, scope, and constraints from the discovery session. Use this to pre-fill the project description and skip clarification questions that the brief already answers.
...
3. **Check Uniqueness**: Verify `{{SPECTRA_DIR}}/specs/` for naming conflicts. If the directory already exists with only `brief.md` (no `spec.json`), use that directory (discovery created it).
4. **Create Directory**: `{{SPECTRA_DIR}}/specs/[feature-name]/` (skip if already exists from discovery)
   ...
   - Write a basic entry for each requirement:
     ...
     mappings:
       - id: "1.1"
         ...
         spec: "{{SPECTRA_DIR}}/specs/{feature-name}/requirements.md"
         design: ""
         code:
           files: []
           symbols: []
...
4. **Next Step**: Command block showing `/spectra-requirements <feature-name>`
