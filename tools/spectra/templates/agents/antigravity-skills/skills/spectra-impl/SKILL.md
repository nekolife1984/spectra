...
- `{{SPECTRA_DIR}}/specs/{feature}/spec.json`, `requirements.md`, `design.md`, `tasks.md`
...
- If review mode is `required`:
  ...
  - The reviewer must apply the `spectra-review` protocol to this task-local review.
...
- If review mode is `inline`:
  - Apply `spectra-review` in the parent context using the same task evidence and the actual `git diff`
...
- If review mode is `off`:
  ...
  - Before marking the task `[x]` or making any success claim, apply `spectra-verify-completion` using fresh evidence from the current code state; then mark task `[x]` in tasks.md and perform selective git commit
- Otherwise:
  ...
  - **APPROVED** → before marking the task `[x]` or making any success claim, apply `spectra-verify-completion` using fresh evidence from the current code state; then mark task `[x]` in tasks.md and perform selective git commit
...
- The debugger must apply the `spectra-debug` protocol to this failure investigation.
...
**`(P)` markers**: Tasks marked `(P)` in tasks.md indicate they have no inter-dependencies and could theoretically run in parallel. However, spectra-impl processes them sequentially (one at a time) to avoid git conflicts and simplify review. The `(P)` marker is informational for task planning, not an execution directive.
...
- **REVIEW**:
  - `required`: Apply `spectra-review` before marking the task complete. If the host supports fresh sub-agents in manual mode, use a fresh reviewer; otherwise perform the review in the main context using the `spectra-review` protocol. Do NOT continue until the verdict is parseably `APPROVED`.
  - `inline`: Apply `spectra-review` in the main context before marking the task complete.
  - `off`: Skip task-local review, but note that `spectra-validate-impl` becomes the primary quality gate before any feature-level completion claim.
- **MARK COMPLETE**:
  - `required|inline`: Only after review returns `APPROVED`, apply `spectra-verify-completion`, then update the checkbox from `- [ ]` to `- [x]` in tasks.md.
  - `off`: Apply `spectra-verify-completion`, then update the checkbox from `- [ ]` to `- [x]` in tasks.md.
