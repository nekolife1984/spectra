...
const manifestPath = join(repoRoot, 'tools/spectra/templates/manifests/opencode-skills.json');
...
describe('real opencode-skills manifest', () => {
  ...
  it('apply writes AGENTS.md, skill files, and settings to cwd', async () => {
    ...
    const skillSpecInit = join(cwd, '.opencode/skills/spectra-init/SKILL.md');
    ...
    const skillSpecQuick = join(cwd, '.opencode/skills/spectra-quick/SKILL.md');
    ...
    const skillSpecDesign = join(cwd, '.opencode/skills/spectra-design/SKILL.md');
    ...
    const skillValidateImpl = join(cwd, '.opencode/skills/spectra-validate-impl/SKILL.md');
    ...
    const skillImpl = join(cwd, '.opencode/skills/spectra-impl/SKILL.md');
    ...
    const skillValidateDesign = join(cwd, '.opencode/skills/spectra-validate-design/SKILL.md');
    ...
    const skillValidateGap = join(cwd, '.opencode/skills/spectra-validate-gap/SKILL.md');
    ...
    const skillSpecRequirements = join(cwd, '.opencode/skills/spectra-requirements/SKILL.md');
    ...
    for (const rule of designRules) {
      expect(await exists(join(cwd, `.opencode/skills/spectra-design/rules/${rule}`))).toBe(true);
    }
    expect(await exists(join(cwd, '.opencode/skills/spectra-validate-design/rules/design-review.md'))).toBe(true);
    expect(await exists(join(cwd, '.opencode/skills/spectra-requirements/rules/ears-format.md'))).toBe(true);
    expect(await exists(join(cwd, '.opencode/skills/spectra-requirements/rules/requirements-review-gate.md'))).toBe(true);
    expect(await exists(join(cwd, '.opencode/skills/spectra-validate-gap/rules/gap-analysis.md'))).toBe(true);
    expect(await exists(join(cwd, '.opencode/skills/spectra-steering/rules/steering-principles.md'))).toBe(true);
    expect(await exists(join(cwd, '.opencode/skills/spectra-steering-custom/rules/steering-principles.md'))).toBe(true);
    expect(await exists(join(cwd, '.opencode/skills/spectra-tasks/rules/tasks-generation.md'))).toBe(true);
    expect(await exists(join(cwd, '.opencode/skills/spectra-tasks/rules/tasks-parallel-analysis.md'))).toBe(true);
    const skillSpecTasks = join(cwd, '.opencode/skills/spectra-tasks/SKILL.md');
    ...
    const tasksGenerationRules = await readFile(join(cwd, '.opencode/skills/spectra-tasks/rules/tasks-generation.md'), 'utf8');
    ...
    const designReviewGate = await readFile(join(cwd, '.opencode/skills/spectra-design/rules/design-review-gate.md'), 'utf8');
    ...
    const requirementsReviewGate = await readFile(join(cwd, '.opencode/skills/spectra-requirements/rules/requirements-review-gate.md'), 'utf8');
  ...
  it('generates exactly 17 skill directories', async () => {
    ...
    const implPrompt = join(cwd, '.opencode/skills/spectra-impl/templates/implementer-prompt.md');
    ...
    const reviewPrompt = join(cwd, '.opencode/skills/spectra-impl/templates/reviewer-prompt.md');
