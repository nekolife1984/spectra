...
describe('real windsurf manifest', () => {
  it('dry-run prints plan for windsurf.json with placeholders applied (mac)', async () => {
    ...
    const manifestPath = join(repoRoot, 'tools/spectra/templates/manifests/windsurf.json');
    const ctx = makeIO();
  ...
  it('shows windsurf recommendation message after applying plan', async () => {
    ...
    expect(out).toContain('Launch Windsurf IDE and run `/spectra-init <what-to-build>` to create a new specification.');
