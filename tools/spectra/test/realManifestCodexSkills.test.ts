...
const manifestPath = join(repoRoot, 'tools/spectra/templates/manifests/codex-skills.json');
...
describe('real codex-skills manifest', () => {
  ...
  it('apply writes AGENTS.md, skill files, and settings to cwd', async () => {
    ...
    const skillSpecInit = join(cwd, '.agents/skills/spectra-init/SKILL.md');
    ...
    const skillSpecDesign = join(cwd, '.agents/skills/spectra-design/SKILL.md');
    ...
    const skillValidateImpl = join(cwd, '.agents/skills/spectra-validate-impl/SKILL.md');
    ...
    expect(skillValidateImplText).toContain('spectra-verify-completion');
    ...
    const skillImpl = join(cwd, '.agents/skills/spectra-impl/SKILL.md');
    ...
    const skillValidateDesign = join(cwd, '.agents/skills/spectra-validate-design/SKILL.md');
    ...
    const skillValidateGap = join(cwd, '.agents/skills/spectra-validate-gap/SKILL.md');
    ...
    const skillSpecRequirements = join(cwd, '.agents/skills/spectra-requirements/SKILL.md');
    ...
    for (const rule of designRules) {
      expect(await exists(join(cwd, `.agents/skills/spectra-design/rules/${rule}`))).toBe(true);
    }
    expect(await exists(join(cwd, '.agents/skills/spectra-validate-design/rules/design-review.md'))).toBe(true);
    expect(await exists(join(cwd, '.agents/skills/spectra-requirements/rules/ears-format.md'))).toBe(true);
    expect(await exists(join(cwd, '.agents/skills/spectra-requirements/rules/requirements-review-gate.md'))).toBe(true);
    expect(await exists(join(cwd, '.agents/skills/spectra-validate-gap/rules/gap-analysis.md'))).toBe(true);
    expect(await exists(join(cwd, '.agents/skills/spectra-steering/rules/steering-principles.md'))).toBe(true);
    expect(await exists(join(cwd, '.agents/skills/spectra-steering-custom/rules/steering-principles.md'))).toBe(true);
    expect(await exists(join(cwd, '.agents/skills/spectra-tasks/rules/tasks-generation.md'))).toBe(true);
    expect(await exists(join(cwd, '.agents/skills/spectra-tasks/rules/tasks-parallel-analysis.md'))).toBe(true);
    const skillSpecTasks = join(cwd, '.agents/skills/spectra-tasks/SKILL.md');
    ...
    const tasksGenerationRules = await readFile(join(cwd, '.agents/skills/spectra-tasks/rules/tasks-generation.md'), 'utf8');
    ...
    const designReviewGate = await readFile(join(cwd, '.agents/skills/spectra-design/rules/design-review-gate.md'), 'utf8');
    ...
    const requirementsReviewGate = await readFile(join(cwd, '.agents/skills/spectra-requirements/rules/requirements-review-gate.md'), 'utf8');
  ...
  it('generates exactly 17 skill directories', async () => {
    ...
    const implPrompt = join(cwd, '.agents/skills/spectra-impl/templates/implementer-prompt.md');
    ...
    const reviewPrompt = join(cwd, '.agents/skills/spectra-impl/templates/reviewer-prompt.md');
    ...
    const debugPrompt = join(cwd, '.agents/skills/spectra-impl/templates/debugger-prompt.md');
    ...
    const skillReview = join(cwd, '.agents/skills/spectra-review/SKILL.md');
    ...
    const skillDebug = join(cwd, '.agents/skills/spectra-debug/SKILL.md');
    ...
    const skillVerifyCompletion = join(cwd, '.agents/skills/spectra-verify-completion/SKILL.md');
