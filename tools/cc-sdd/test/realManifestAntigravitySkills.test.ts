...
describe('real antigravity-skills manifest', () => {
  ...
  it('apply writes AGENTS.md, skill files, and settings to cwd', async () => {
    ...
    const skillSpecInit = join(cwd, '.agent/skills/spec-init/SKILL.md');
    ...
    const skillSpecQuick = join(cwd, '.agent/skills/spec-quick/SKILL.md');
    ...
    const skillSpecDesign = join(cwd, '.agent/skills/spec-design/SKILL.md');
    ...
    const skillValidateImpl = join(cwd, '.agent/skills/spec-validate-impl/SKILL.md');
    ...
    const skillImpl = join(cwd, '.agent/skills/spec-impl/SKILL.md');
    ...
    const skillValidateDesign = join(cwd, '.agent/skills/spec-validate-design/SKILL.md');
    ...
    const skillValidateGap = join(cwd, '.agent/skills/spec-validate-gap/SKILL.md');
    ...
    const skillSpecRequirements = join(cwd, '.agent/skills/spec-requirements/SKILL.md');
    ...
    for (const rule of designRules) {
      expect(await exists(join(cwd, `.agent/skills/spec-design/rules/${rule}`))).toBe(true);
    }
    expect(await exists(join(cwd, '.agent/skills/spec-validate-design/rules/design-review.md'))).toBe(true);
    expect(await exists(join(cwd, '.agent/skills/spec-requirements/rules/ears-format.md'))).toBe(true);
    expect(await exists(join(cwd, '.agent/skills/spec-requirements/rules/requirements-review-gate.md'))).toBe(true);
    expect(await exists(join(cwd, '.agent/skills/spec-validate-gap/rules/gap-analysis.md'))).toBe(true);
    expect(await exists(join(cwd, '.agent/skills/spec-steering/rules/steering-principles.md'))).toBe(true);
    expect(await exists(join(cwd, '.agent/skills/spec-steering-custom/rules/steering-principles.md'))).toBe(true);
    expect(await exists(join(cwd, '.agent/skills/spec-tasks/rules/tasks-generation.md'))).toBe(true);
    expect(await exists(join(cwd, '.agent/skills/spec-tasks/rules/tasks-parallel-analysis.md'))).toBe(true);
    const skillSpecTasks = join(cwd, '.agent/skills/spec-tasks/SKILL.md');
    ...
    const tasksGenerationRules = await readFile(join(cwd, '.agent/skills/spec-tasks/rules/tasks-generation.md'), 'utf8');
    ...
    const designReviewGate = await readFile(join(cwd, '.agent/skills/spec-design/rules/design-review-gate.md'), 'utf8');
    ...
    const requirementsReviewGate = await readFile(join(cwd, '.agent/skills/spec-requirements/rules/requirements-review-gate.md'), 'utf8');
  ...
  it('generates exactly 17 skill directories', async () => {
    ...
    const implPrompt = join(cwd, '.agent/skills/spec-impl/templates/implementer-prompt.md');
    ...
    const reviewPrompt = join(cwd, '.agent/skills/spec-impl/templates/reviewer-prompt.md');
