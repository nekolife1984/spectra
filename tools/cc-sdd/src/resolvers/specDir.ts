...
export const defaultSpecDir = '.spec';

export const resolveSpecDir = (opts: SpecDirOptions = {}): string => {
  const candidate = opts.flag ?? opts.config ?? defaultSpecDir;

  if (!candidate || typeof candidate !== 'string') {
    throw new Error('specDir must be a non-empty string');
  }

  // Reject absolute paths
  if (path.isAbsolute(candidate)) {
    throw new Error('specDir must be a repository-relative path');
  }
  ...
  if (!allowed.test(candidate)) {
    throw new Error('specDir contains disallowed characters');
  }

  // Reject parent directory traversal
  const segments = candidate.split(/[\\/]+/);
  if (segments.some((s) => s === '..')) {
    throw new Error('specDir must not contain parent traversal (..)');
  }
