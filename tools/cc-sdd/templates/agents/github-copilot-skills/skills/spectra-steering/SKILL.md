---
name: spectra-steering
description: Manage {{SPECTRA_DIR}}/steering/ as persistent project knowledge
metadata:
...
**Role**: Maintain `{{SPECTRA_DIR}}/steering/` as persistent project memory.
...
- All `{{SPECTRA_DIR}}/steering/*.md` treated equally (core + custom)
</background_information>

<instructions>
## Scenario Detection

Check `{{SPECTRA_DIR}}/steering/` status:
...
1. Load all existing steering (`{{SPECTRA_DIR}}/steering/*.md`)
...
- All `{{SPECTRA_DIR}}/steering/*.md` loaded as project memory
...
- Light references to `{{SPECTRA_DIR}}/specs/` and `{{SPECTRA_DIR}}/steering/` are acceptable; avoid other `.spectra/` directories
