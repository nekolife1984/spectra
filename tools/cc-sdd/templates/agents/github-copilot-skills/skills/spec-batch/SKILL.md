...
1. Read `{{SPEC_DIR}}/steering/roadmap.md`
...
4. For each pending feature in `## Specs (dependency order)`, verify `{{SPEC_DIR}}/specs/<feature>/brief.md` exists
5. If any brief.md is missing, stop and report: "Missing brief.md for: [list]. Run `$spec-discovery` to generate briefs first."
...
1. Read the brief at {{SPEC_DIR}}/specs/{feature-name}/brief.md for feature context
2. Read the roadmap at {{SPEC_DIR}}/steering/roadmap.md for project context
3. Execute the full spec pipeline. For each phase, read the corresponding skill's SKILL.md for complete instructions (templates, rules, review gates):
   a. Initialize: Read .agents/skills/spec-init/SKILL.md, then create spec.json and requirements.md
   b. Generate requirements: Read .agents/skills/spec-requirements/SKILL.md, then follow its steps
   c. Generate design: Read .agents/skills/spec-design/SKILL.md, then follow its steps
   d. Generate tasks: Read .agents/skills/spec-tasks/SKILL.md, then follow its steps
...
- `{{SPEC_DIR}}/specs/*/design.md` (primary: contains interfaces, data models, architecture)
- `{{SPEC_DIR}}/specs/*/requirements.md` (for scope and acceptance criteria)
- `{{SPEC_DIR}}/specs/*/tasks.md` (for boundary annotations only -- read _Boundary:_ lines, skip task descriptions)
...
1. Scan `{{SPEC_DIR}}/specs/*/tasks.md` to verify all specs exist
...
Next: Review generated specs, then start implementation with $spec-impl <feature>
```
...
- Suggest: "Run `$spec-quick <feature> --auto` manually for failed features."
...
**Roadmap not found**:
- Stop and report: "No roadmap.md found. Run `$spec-discovery` first."
