...
- `{{SPEC_DIR}}/specs/$1/spec.json`, `requirements.md`, `design.md`
- `{{SPEC_DIR}}/specs/$1/tasks.md` (if exists, for merge mode)
...
- Read `{{SPEC_DIR}}/settings/templates/specs/tasks.md` for format (supports `(P)` markers)
...
**Write tasks.md**:
- Create/update `{{SPEC_DIR}}/specs/$1/tasks.md`
...
- If auto-approve flag (`-y`) is provided:
  ...
  - Respond: "Tasks generated and auto-approved. Start implementation with `$spec-impl $1`"
- Otherwise (interactive):
  ...
  - If the user approves:
    - Set `approvals.tasks.approved: true` in spec.json
    - Respond: "Tasks approved. Start implementation with `$spec-impl $1`"
...
1. **Status**: Confirm tasks generated at `{{SPEC_DIR}}/specs/$1/tasks.md`
2. **Task Summary**: 
...
- **User Message**: "Missing requirements.md or design.md at `{{SPEC_DIR}}/specs/$1/`"
...
- **Suggested Action**: "Refine requirements.md or design.md, then re-run `$spec-tasks $1`"

**Template/Rules Missing**:
- **User Message**: "Template or rules files missing in `{{SPEC_DIR}}/settings/`"
...
- Autonomous implementation: `$spec-impl $1`
- Specific tasks only: `$spec-impl $1 1.1,1.2`
