...
- Read `{{SPEC_DIR}}/specs/$1/spec.json` for metadata and phase status
- Read `{{SPEC_DIR}}/specs/$1/brief.md` if it exists
- Read existing files: `requirements.md`, `design.md`, `tasks.md` (if they exist)
- Check `{{SPEC_DIR}}/specs/$1/` directory for available files
- Read `{{SPEC_DIR}}/steering/roadmap.md` if it exists and this spec appears in it
...
**Spec Not Found**:
- **Message**: "No spec found for `$1`. Check available specs in `{{SPEC_DIR}}/specs/`"
...
- Shows all specs in `{{SPEC_DIR}}/specs/` with their status
