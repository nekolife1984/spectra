...
## Execution Steps
1. **Check for Brief**: If `{{SPEC_DIR}}/specs/{feature-name}/brief.md` exists (created by `$spec-discovery`), read it. The brief contains problem, approach, scope, and constraints from the discovery session. Use this to pre-fill the project description and skip clarification questions that the brief already answers.
...
3. **Check Uniqueness**: Verify `{{SPEC_DIR}}/specs/` for naming conflicts. If the directory already exists with only `brief.md` (no `spec.json`), use that directory (discovery created it).
4. **Create Directory**: `{{SPEC_DIR}}/specs/[feature-name]/` (skip if already exists from discovery)
5. **Initialize Files Using Templates**:
   - Read `{{SPEC_DIR}}/settings/templates/specs/init.json`
   - Read `{{SPEC_DIR}}/settings/templates/specs/requirements-init.md`
   ...
   - Write a basic entry for each requirement:
     ...
     mappings:
       - id: "1.1"
         ...
         spec: "{{SPEC_DIR}}/specs/{feature-name}/requirements.md"
         design: ""
         code:
           files: []
           symbols: []
...
- **Template Missing**: If template files don't exist in `{{SPEC_DIR}}/settings/templates/specs/`, report error with specific missing file path and suggest checking repository setup
