...
- `{{SPEC_DIR}}/specs/{feature}/spec.json`, `requirements.md`, `design.md`, `tasks.md`
...
- If review mode is `required`:
  ...
  - The reviewer must apply the `spec-review` protocol to this task-local review.
...
- If review mode is `inline`:
  - Apply `spec-review` in the parent context using the same task evidence and the actual `git diff`
...
- If review mode is `off`:
  ...
  - Before marking the task `[x]` or making any success claim, apply `spec-verify-completion` using fresh evidence from the current code state; then mark task `[x]` in tasks.md and perform selective git commit
- Otherwise:
  ...
  - **APPROVED** → before marking the task `[x]` or making any success claim, apply `spec-verify-completion` using fresh evidence from the current code state; then mark task `[x]` in tasks.md and perform selective git commit
...
5. **Run trace completeness gate**: `python3 .agents/scripts/check-trace-completeness.py` — verify all trace elements. If it fails, fix the gaps before proceeding.
...
- The debugger must apply the `spec-debug` protocol to this failure investigation.
...
**`(P)` markers**: Tasks marked `(P)` in tasks.md indicate they have no inter-dependencies and could theoretically run in parallel. However, spec-impl processes them sequentially (one at a time) to avoid git conflicts and simplify review. The `(P)` marker is informational for task planning, not an execution directive.
...
- **REVIEW**:
  - `required`: Apply `spec-review` before marking the task complete. If the host supports fresh sub-agents in manual mode, use a fresh reviewer; otherwise perform the review in the main context using the `spec-review` protocol. Do NOT continue until the verdict is parseably `APPROVED`.
  - `inline`: Apply `spec-review` in the main context before marking the task complete.
  - `off`: Skip task-local review, but note that `spec-validate-impl` becomes the primary quality gate before any feature-level completion claim.
- **MARK COMPLETE**:
  - `required|inline`: Only after review returns `APPROVED`, apply `spec-verify-completion`, then update the checkbox from `- [ ]` to `- [x]` in tasks.md.
  - `off`: Apply `spec-verify-completion`, then update the checkbox from `- [ ]` to `- [x]` in tasks.md.

## Step 4: Final Validation

**Autonomous mode**:
- After all tasks complete, run `$spec-validate-impl $1` as a GO/NO-GO gate
- If validation returns GO → before reporting feature success, apply `spec-verify-completion` to the feature-level claim using the validation result and fresh supporting evidence
...
**Manual mode**:
- Suggest running `$spec-validate-impl $1` but do not auto-execute
- If review mode is `off`, treat `$spec-validate-impl $1` as mandatory before any feature-level success claim
...
**Session Interrupted**:
- Safe to re-run `$spec-impl $1` — completed tasks are already `[x]` in tasks.md and committed to git
