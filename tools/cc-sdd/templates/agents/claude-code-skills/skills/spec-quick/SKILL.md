...
1. **Check for Brief**:
   - If `{{SPEC_DIR}}/specs/{feature-name}/brief.md` exists (created by `$spec-discovery`), read it for discovery context (problem, approach, scope, constraints)
...
3. **Check Uniqueness**:
   - Use Glob to check `{{SPEC_DIR}}/specs/*/`
...
4. **Create Directory**:
   - Use Bash: `mkdir -p {{SPEC_DIR}}/specs/{feature-name}` (skip if already exists from discovery)

5. **Initialize Files from Templates**:

   a. Read templates:
   ```
   - {{SPEC_DIR}}/settings/templates/specs/init.json
   - {{SPEC_DIR}}/settings/templates/specs/requirements-init.md
   ```
   ...
   ```
   - {{SPEC_DIR}}/specs/{feature-name}/spec.json
   - {{SPEC_DIR}}/specs/{feature-name}/requirements.md
   ```

6. **Output Progress**: "Phase 1/4 complete: Spec initialized at {{SPEC_DIR}}/specs/{feature-name}/"
...
Invoke `$spec-requirements {feature-name}`.
...
Invoke `$spec-design {feature-name} -y`. The `-y` flag auto-approves requirements.
...
Invoke `$spec-tasks {feature-name} -y`. The `-y` flag auto-approves requirements, design, and tasks.
...
```
Spec initialized at {{SPEC_DIR}}/specs/{feature}/
...
## Generated Files:
- {{SPEC_DIR}}/specs/{feature}/spec.json
- {{SPEC_DIR}}/specs/{feature}/requirements.md ({X} requirements)
- {{SPEC_DIR}}/specs/{feature}/design.md ({Y} components, {Z} endpoints)
- {{SPEC_DIR}}/specs/{feature}/tasks.md ({N} tasks)

Quick generation skipped:
- `$spec-validate-gap` - Gap analysis (integration check)
- `$spec-validate-design` - Design review (architecture validation)
...
2. Optional validation:
   - `$spec-validate-gap {feature}` - Check integration with existing codebase
   - `$spec-validate-design {feature}` - Verify architecture quality
...
**Template Missing**:
- Check `{{SPEC_DIR}}/settings/templates/specs/` exists
...
- Suggest targeted follow-up with `$spec-design {feature}`, `$spec-tasks {feature}`, or manual edits depending on the finding
